(function($) {
    ...
    $(window).on('resize', function (e) {
        ...
        var sideMaskBackgroundWidth = parseInt(($('#page-wrapper').width() - $('#page-container', '#page-wrapper').width()) / 2);
        // TODO: round sideMaskBackgroundWidth to fit the plain width outside #page-container
        $('.side-mask-background', '#page-wrapper').css({
            'width': sideMaskBackgroundWidth + 'px',
    ...
    $(document).ready(function() {
			...
			$.ajax({
				...
				success: function(count){
					...
					if(count != "already")
					{
						...
						//$this.parents('.thumbnail-portfolio').find('.like p').text(count);
						//$this.parents('.thumbnail-portfolio').find('.like .fa-heart').addClass('votedIcon');
					}
				}
			});
        ...
        $(".control-sound").find("img").click(function() {
            if ($(".control-sound").find("img").hasClass("mute")) {
                ...
                $(".control-sound").find("img").attr("src", ajax_var.themeURL+'/images/playing.png');
                $(".control-sound").find("img").removeClass('mute');
                $(".control-sound").find("img").addClass('unmute');
            } else {
                $('#bgndVideo').muteYTPVolume();
                $(".control-sound").find("img").attr("src", ajax_var.themeURL+'/images/mute.png');
		...
		$('.rb-twitter-feed').each(function(){
			var params = '';
			var $this = $(this);
			params = 'action=rb_getTweets&nonce='+ajax_var.nonce+'&limit='+$(this).data('limit')+'&user='+$(this).data('user');
			$.post(ajax_var.url, params, function(data){
				$this.html(data);
		...
		$('.twitter-box').each(function(){
			var params = '';
			var $this = $(this);
			params = 'action=rb_getWidgetTweets&nonce='+ajax_var.nonce+'&limit='+$(this).data('limit')+'&user='+$(this).data('user');
			$.post(ajax_var.url, params, function(data){
				...
				thisForm.fadeOut(function(){
				  thisForm.parent().find('.loading').fadeIn(function(){
					$.ajax({
					  type: 'POST',
					  url: ajax_var.url,
					  data: thisForm.serialize()+'&action=rb_send_contact_form&nonce='+ajax_var.nonce,
					  success: function(data){
						data = $.parseJSON(data);
						if(data.status=='OK'){
