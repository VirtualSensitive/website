...
function rb_update_notifier_menu() {
	$xml = rb_get_latest_theme_version(21600); // This tells the function to cache the remote call for 21600 seconds (6 hours)
	$theme_data =  wp_get_theme('Federal'); // Get theme data from style.css (current version is what we want)
	if(isset($theme_data)){
...
function rb_update_notifier() {
	$xml = rb_get_latest_theme_version(21600); // This tells the function to cache the remote call for 21600 seconds (6 hours)
	$theme_data =  wp_get_theme('Federal');
	?>
...
function rb_get_latest_theme_version($interval) {
	// remote xml file location
	$notifier_file_url = 'http://renklibeyaz.com/federal.xml';
